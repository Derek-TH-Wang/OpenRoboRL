...
class ImitationTask(object):
    ...
    def reset(self):
        ...
        if self._ref_motions is None:
            self._ref_motions = self._load_ref_motions(
                self._ref_motion_filenames)
    ...
    def build_target_obs(self):
        ...
        inv_heading_rot = transformations.quaternion_about_axis(-heading, [
                                                                0, 0, 1])
    ...
    def _calc_reward_end_effector(self):
        ...
        end_effector_reward = np.exp(
            -self._end_effector_err_scale * end_eff_err)
            ...
            if (j_pose_idx < 0):
                ...
                if (j == 0):
                    ...
                    j_pose_idx = self._joint_pose_idx[j - 1] + self._joint_pose_size[j -
                                                                                     1]
                    j_vel_idx = self._joint_vel_idx[j -
                                                    1] + self._joint_vel_size[j - 1]
    ...
    def _calc_ref_pose(self, time, apply_origin_offset=True):
        ...
        if apply_origin_offset:
            ...
            root_rot = transformations.quaternion_multiply(self._origin_offset_rot,
                                                           root_rot)
            root_pos = pose3d.QuaternionRotatePoint(
                root_pos, self._origin_offset_rot)
    ...
    def _calc_ref_vel(self, time):
        ...
        root_vel = pose3d.QuaternionRotatePoint(
            root_vel, self._origin_offset_rot)
    ...
    def _sync_ref_origin(self, sync_root_position, sync_root_rotation):
        ...
        if sync_root_position:
            ...
            ref_pos = pose3d.QuaternionRotatePoint(
                ref_pos, self._origin_offset_rot)
    ...
    def _reset_clip_change_time(self):
        ...
        if np.isfinite(self._clip_time_min) and np.isfinite(self._clip_time_max):
            clip_dur = self._rand_uniform(
                self._clip_time_min, self._clip_time_max)
            time = self._get_motion_time()
    ...
    def _apply_state_perturb(self, pose, vel):
        ...
        root_ang_vel += self._randn(0, root_ang_vel_std,
